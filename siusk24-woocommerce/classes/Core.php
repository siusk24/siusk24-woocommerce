...
class Core {
	...
	public function show_admin_notice( $msg_text, $msg_type = 'info', $dismissible = true ) {
		add_action(
			'admin_notices',
			function () use ( $msg_text, $msg_type, $dismissible ) {
				$additional_classes = '';
				if ( $dismissible ) {
	...
	public function get_parcels( $order = false ) {
		...
		if ( $send_as_one == 'yes' ) {
			...
			return array( $main_parcel->generateParcel() );
		}
		return $parcels;
	}
	...
	/**
	 * Resolve locker network + matching service from the selected terminal row.
	 *
	 * @param int|string $terminal_id
	 * @param string     $fallback_service
	 * @return array{identifier:string,service_code:string,terminal:?object}
	 */
	public function resolve_locker_from_terminal( $terminal_id, $fallback_service = '' ) {
		$result = array(
			'identifier'   => '',
			'service_code' => (string) $fallback_service,
			'terminal'     => null,
		);

		$terminal_id = absint( $terminal_id );
		if ( ! $terminal_id ) {
			return $result;
		}

		$terminal = Terminal::getById( $terminal_id );
		if ( ! $terminal ) {
			return $result;
		}

		$result['terminal']   = $terminal;
		$result['identifier'] = isset( $terminal->identifier ) ? trim( (string) $terminal->identifier ) : '';
		$matched              = $this->match_service_for_identifier( $result['identifier'], $fallback_service );
		if ( ! empty( $matched ) ) {
			$result['service_code'] = $matched;
		}
	...
	/**
	 * Persist terminal id, network identifier and matching locker service on the order.
	 *
	 ...
	 * @return void
	 */
	public function apply_selected_terminal_to_order( $order, $terminal_id, $fallback_service = '' ) {
		if ( ! $order ) {
			return;
		}
		...
		$resolved = $this->resolve_locker_from_terminal( $terminal_id, $fallback_service );

		$order->update_meta_data( '_siusk24_terminal_id', $terminal_id );
		$order->update_meta_data( '_siusk24_method', 1 );

		if ( $resolved['identifier'] !== '' ) {
			$order->update_meta_data( '_siusk24_identifier', $resolved['identifier'] );
		}

		if ( ! empty( $resolved['service_code'] ) ) {
			$order->update_meta_data( '_siusk24_service', $resolved['service_code'] );
		}
	}

	/**
	 * Map terminal.identifier to an enabled locker service_code.
	 *
	 * @param string $identifier
	 * @param string $fallback_service
	 * @return string
	 */
	public function match_service_for_identifier( $identifier, $fallback_service = '' ) {
		$needles = $this->identifier_search_needles( $identifier );
		if ( empty( $needles ) ) {
			return (string) $fallback_service;
		}

		$this->get_config();
		$enabled  = $this->selected_services();
		$services = $this->api->get_services();
		...
		$matches = array();
		foreach ( $services as $service ) {
			if ( empty( $service->service_code ) || ! empty( $service->delivery_to_address ) ) {
				continue;
			}

			$haystack = $this->normalize_search_text(
				( $service->name ?? '' ) . ' ' . ( $service->additional ?? '' ) . ' ' . ( $service->description ?? '' )
			);

			foreach ( $needles as $needle ) {
				if ( $needle !== '' && $this->text_contains( $haystack, $needle ) ) {
					$matches[] = $service->service_code;
					break;
				}
			}
		}
		...
		if ( $fallback_service && in_array( $fallback_service, $matches, true ) ) {
			return (string) $fallback_service;
		}
	...
	private function identifier_search_needles( $identifier ) {
		$id = $this->normalize_search_text( $identifier );
		if ( $id === '' ) {
			return array();
		}

		$map = array(
			'gls'        => array( 'smartposti', 'gls' ),
			'smartposti' => array( 'smartposti', 'gls' ),
			'venipak'    => array( 'venipak' ),
			'dpd'        => array( 'dpd' ),
			'lppost'     => array( 'lietuvos pastas', 'lietuvos paštas', 'lp express', 'lpexpress', 'lppost' ),
			'lp_express' => array( 'lp express', 'lpexpress', 'lietuvos pastas', 'lietuvos paštas' ),
			'omniva'     => array( 'omniva' ),
		);
	...
	private function text_contains( $haystack, $needle ) {
		$needle = $this->normalize_search_text( $needle );
		if ( $needle === '' ) {
			return false;
		}
	...
	private function calculate_price( $price, $type, $value ) {
		...
		if ( $type == 'fixed' ) {
			$price = $value;
		} elseif ( $type == 'addition_percent' ) {
			$price += round( $price * $value / 100, 2 );
		} elseif ( $type == 'addition_eur' ) {
			$price += $value;
		}
		return $price;
	}
	...
	public function register_order( $params ) {
		...
		try {
			...
			if ( ! $allow_regenarate && ! empty( $shipment_id ) ) {
				return array(
					'status' => 'error',
					'msg'    => __( 'The shipment is already registered', 'siusk24' ),
				);
			}
			...
			if ( $terminal ) {
				$resolved = $this->resolve_locker_from_terminal( $terminal, $service_code );
				if ( ! empty( $resolved['service_code'] ) ) {
					$service_code = $resolved['service_code'];
				}
				$terminal_obj = ! empty( $resolved['terminal'] ) ? $resolved['terminal'] : Terminal::getById( $terminal );
				if ( ! $terminal_obj || ! $terminal_obj->zip ) {
					return array(
						'status' => 'error',
						'msg'    => __( 'Terminal not found', 'siusk24' ),
					);
				}
	...
	public function print_labels( $shipments_ids ) {
		try {
			...
			if ( empty( $labels ) ) {
				return array(
					'status' => 'error',
					'msg'    => __( 'Failed to get labels', 'siusk24' ),
				);
			}
	...
	private function merge_pdfs( $pdf_files ) {
		...
		foreach ( $pdf_files as $file ) {
			...
			for ( $page_no = 1; $page_no <= $page_count; $page_no++ ) {
				...
				$pdf->AddPage( 'P' );
				$pdf->useTemplate( $template_id, array( 'adjustPageSize' => true ) );
			}
		}
